...
async def add(event):
    ok = await eor(event, "**⌛ 𝙰𝚍𝚍𝚒𝚗𝚐 𝚂𝚞𝚍𝚘 𝚄𝚜𝚎𝚛𝚜 in data base...**")
    bot = "SUDO_USERS"
...
async def remove_sudo(event):
    Heroku = heroku3.from_key(JARVIS)
    app = Heroku.app(LOP)
    heroku_var = app.config()
    if not event.is_reply:
        return await event.edit("**Removing user from sudo list**")
    if event.is_reply:
        ...
        if op:
            ...
            await event.edit(
                f"user **{name}** is removed from sudo"
            )
        else:
            await event.edit(f"User {name} not found in sudo")
        if heroku_var["SUDO_USERS"] == None:
            await event.edit(f"Sudo is disable")


@bot.on(admin_cmd("listsudo"))
async def sudolists(event):
    op = await event.edit("Scanning data base")
    Heroku = heroku3.from_key(JARVIS)
    app = Heroku.app(LOP)
    app.config()
    if not sudousers:
        return await event.edit("No sudo user found")
