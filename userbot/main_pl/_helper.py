...
async def repo(event):
    try:
        jarvis = await bot.inline_query(botname, "repo")
        await jarvis[0].click(event.chat_id)
        if event.sender_id == The_jarvisBoy:
            await event.delete()
    except (noin, dedbot):
        await eor(event, msg)
        ...
        try:
            ...
            async with bot.conversation(chat) as conv:
                try:
                    ...
                    return await jarvis.edit("Unblock @Botfather first.")
                await jarvis.edit(
...
async def yardim(event):
    ...
    if tgbotusername is not None or jarvis_input == "text":
        results = await event.client.inline_query(tgbotusername, "jarvisbot_help")
        await results[0].click(
            event.chat_id, reply_to=event.reply_to_msg_id, hide_via=True
        )
...
async def _(event):
    ...
    await event.edit(" Mr.Starks Assistant- Jarvis ")
