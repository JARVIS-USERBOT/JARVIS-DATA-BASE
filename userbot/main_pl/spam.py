...
async def bigspam(JARVIS):
    if not JARVIS.text[0].isalpha() and JARVIS.text[0] not in ("/", "#", "@", "!"):
        JARVIS_msg = JARVIS.text
        count = int(JARVIS_msg[9:13])
        spam = str(JARVIS.text[13:])
        for i in range(1, count):
            await JARVIS.respond(spam)
        await JARVIS.delete()
        if LOGGER:
