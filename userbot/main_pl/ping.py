...
tony = borg.uid
JARVIS_IMG = os.environ.get("PING_PIC", "https://te.legra.ph/file/eb0460fd8f4436d722c92.jpg")

@bot.on(admin_cmd(pattern=f"aping$", outgoing=True))
@bot.on(sudo_cmd(pattern=f"aping$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    start = datetime.now()
...
async def _(event):
    ...
    ms = (end - start).microseconds / 1000
    if JARVIS_IMG:
        jarvis_caption = f"**Ping**\n\n   🔸️ {ms}\n   🔹️ **𝙼𝚢** **𝙼𝚊𝚜𝚝𝚎𝚛** ~『[{DEFAULTUSER}](tg://user?id={tony})』"
        await event.client.send_file(
            event.chat_id, JARVIS_IMG, caption=jarvis_caption
        )
        await event.delete()
