...
async def nope(JARVIS):
    JARVIS = JARVIS.pattern_match.group(1)
    if not JARVIS:
        if JARVIS.is_reply:
            (await JARVIS.get_reply_message()).message
        else:
            await edit_or_reply(JARVIS, "I need some text to make a tweet🚶")
            return
    tweeter = await bot.inline_query("TwitterStatusBot", f"{(deEmojify(JARVIS))}")
    await tweeter[0].click(
        JARVIS.chat_id,
        reply_to=JARVIS.reply_to_msg_id,
        silent=True if JARVIS.is_reply else False,
        hide_via=True,
    )
    await JARVIS.delete()
