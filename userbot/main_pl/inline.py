...
jarvis_pic = Config.PM_PIC or "https://te.legra.ph/file/eb0460fd8f4436d722c92.jpg"
...
if Config.BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if event.query.user_id == bot.uid and query == "jarvisbot_help":
            ...
            jarvis = hunter.split("+")
            user = await bot.get_entity(int(jarvis[0]))
            channel = await bot.get_entity(int(jarvis[1]))
            ...
            pyt_thon = alive_txt.format(Config.ALIVE_MSG, alive_emoji, jarvis_mention, alive_emoji, jarvisversion, alive_emoji, version.__version__, alive_emoji, abuse_m, alive_emoji, is_sudo, alive_emoji, Config.BOY_OR_GIRL)
            alv_btn = [
                [Button.url(f"{jarvis_USER}", f"tg://openmessage?user_id={Its_py}")],
            ...
            pyth_on = jarvis_FIRST.format(jarvis_mention, mssge)
            result = builder.photo(
                file=jarvis_pic,
                text=pyth_on,
                buttons=[
            ...
            result = builder.article(
                title="Repository",
                text=f"**⚜ OP JARVIS BOT ⚜**",
                buttons=[
                    [Button.url("♥️ **Repo** ♥", "https://github.com/JARVIS-USERBOT/JARVIS-BOT")],
                    [Button.url("♦️ **Creator** ♦️", "https://t.me/Its_py")],
                ],
            )
            ...
            result = builder.article(
                "@jarvis_Userbot_Support",
                text="""**Hey! This is [jarvisẞø†](https://t.me/JarvisUserBot_Support) \nYou can know more about me from the links given below 👇**""",
                buttons=[
                    [
                        custom.Button.url("🔥 CHANNEL 🔥", "https://t.me/Jarvis_Support_Official"),
                        custom.Button.url(
                            "⚡ GROUP ⚡", "https://t.me/JarvisUserBot_Support"
                        ),
                    ],
                    [
                        custom.Button.url(
                            "⚡ REPO ⚡", "https://github.com/JARVIS-USERBOT/JARVISBOT"),
                        custom.Button.url
                    (
                            "⚡ TUTORIAL ⚡", "https://youtu.be/gkVIksTnERI"
                    )
                    ],
                ],
                link_preview=False,
            )
        await event.answer([result] if result else Nonet)
    ...
    async def on_pm_click(event):
        if event.query.user_id == bot.uid:
            ...
            await event.edit(
                f"🔰 This is jarvisẞø† PM Security for {jarvis_mention} to keep away unwanted retards from spamming PM..."
            )
    ...
    async def page(event):
        ...
        if event.query.user_id == bot.uid or event.query.user_id in Config.SUDO_USERS:
            await event.edit(
                f"**{jarvis_emoji2}{jarvis_mention}{jarvis_emoji1}\n\n**🕹️**Files in Data Base**⭆ `{len(CMD_HELP)}`**\n** **Commands**⭆ `{len(apn)}`**\n** **🎒Current page⭆ 1/{veriler[0]}**",
                buttons=veriler[1],
                link_preview=False,
            )
        else:
            return await event.answer(
                "This Is Jarvis . Made For Tony Stark . If you want deploy your own.",
                cache_time=0,
                alert=True,
            )
    ...
    async def Information(event):
        ...
        buttons.append([custom.Button.inline(f"{jarvis_emoji1} Main Menu {jarvis_emoji2}", data=f"page({page})")])
        if event.query.user_id == bot.uid or event.query.user_id in Config.SUDO_USERS:
            await event.edit(
    ...
    async def commands(event):
        ...
        if event.query.user_id == bot.uid or event.query.user_id in Config.SUDO_USERS:
            await event.edit(
                result,
                buttons=[
                    custom.Button.inline(f"{jarvis_emoji1} Return {jarvis_emoji2}", data=f"Information[{page}]({cmd})")
                ],
                link_preview=False,
            )
        else:
