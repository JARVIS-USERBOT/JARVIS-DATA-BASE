...
if Config.TAG_LOGGER:
    ...
    async def all_messages_catcher(event):
        ...
        jarvis = await event.client.get_entity(event.sender_id)
        if jarvis.bot or jarvis.verified or jarvis.support:
            return

        legendm = f"[{get_display_name(jarvis)}](tg://user?id={jarvis.id})"
