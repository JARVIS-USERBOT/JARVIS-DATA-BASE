...
from JARVISBOT.utils import admin_cmd, eor, sudo_cmd
from userbot import HEROKU_APP, JARVISversion, bot
from userbot.cmdhelp import CmdHelp
from userbot.helpers.runner import reload_JARVISBOT
...
async def _(event):
    ...
    await event.edit("яєϐοοτιиg **[ ███ ]** ...\nωαιτ ƒєω мιиυτєѕ☢️")
    await event.edit(
        f"REBOOTED AND UPDATED JARVIS-USERBOT {JARVISversion} **[ ✔️ ]** ...\nType `.ping` or `.up` after 5min to check if I am working✔️"
    )
...
async def rel(event):
    await eor(event, "Reloading JARVISBOT... Wait for few seconds...")
    await reload_JARVISBOT()
