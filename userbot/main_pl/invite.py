...
from JARVISBOT.utils import admin_cmd, edit_or_reply, eor, sudo_cmd
...
async def get_users(event):
    ...
    JARVIS_chat = legen_.lower
    restricted = ["@JARVIS_Userbot", "@Official_JARVISBot"]
    JARVIS = await eor(event, f"**Inviting members from** {legen_}")
    ...
    await JARVIS.edit(
        "**⚜️[Ͳєямιиαℓ Տτατυѕ](https://t.me/JARVIS_Userbot)**\n\n`🔸Inviting Users.......`"
    )
...
async def get_users(event):
    ...
    chat = await event.get_chat()
    if event.is_private:
        return await JARVIS.edit_or_reply("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"

    await JARVIS.edit_or_reply("**TerminalStatus**\n\n`Collecting Users.......`")
    async for user in event.client.iter_participants(aura.full_chat.id):
        try:
            if error.startswith("Too"):
                return await JARVIS.edit_or_reply(
