...
async def variable(var):
    ...
    if exe == "get":
        ...
        try:
            ...
            if "JARVIS_STRING" in variable:
                await eor(
                    var, "SAFETY PROTOCAL DONT ALLOW ME TO DO SO \n @JarvisUserBot_Support"
                )
                return
            elif variable in heroku_var:
                await eor(var, JARVIS)
            else:
                return await var.edit(
                    "**ConfigVars**:" f"\n\n`Error:\n-> {variable} not in data-base`"
                )
        except IndexError:
        ...
        if variable in heroku_var:
            ...
            return await var.edit(f"**{variable}**  ` not found in data-base`")


@bot.on(admin_cmd(pattern="life(?: |$)", outgoing=True))
@bot.on(sudo_cmd(pattern="life(?: |$)", allow_sudo=True))
...
async def _(event):
    if (HEROKU_APP_NAME is None) or (HEROKU_API_KEY is None):
        return await eor(
            dyno,
            f"**Error occured** \n Check You HEROKU_APP_NAME and HEROKU_API_KEY in config \n Go to {my_group} for help.",
            link_preview=False,
        )
    try:
    ...
    JARVIS_data = app.get_log()
    await eor(event, JARVIS_data)


CmdHelp("heroku").add_command(
    "usage", None, "Check your bot life"
).add_command(
    "set var",
    "<NEW VAR> <value>",
