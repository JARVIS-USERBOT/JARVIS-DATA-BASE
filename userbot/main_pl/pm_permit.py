...
from userbot.main_pl.sql_helper import pm_permit_sql as pm_sql
from . import *
jarvis_mention = borg.uid

WARN_PIC = Config.PM_PIC or "https://te.legra.ph/file/eb0460fd8f4436d722c92.jpg"
...
LEGEND_FIRST = (
    "**🔥 Jarvis Prîvã†é Sêçürïty Prø†öçõl 🔥**\n\nThis is to inform you that "
    "{} is currently unavailable.\nThis is an automated message.\n\n"
    "{}\n\n**{}Please Choose Why You Are Here!!**".format(jarvis_mention, CSTM_PMP, max_flood)
)
...
if PM_ON_OFF != "DISABLE":
    ...
    async def approve_p_m(event):
        ...
        chat = await event.get_chat()
        if event.is_private:
            if chat.id == 1718517513:
                await event.edit(
                    "You tried to block my owner😡. GoodBye for 100 seconds!🥱😴😪💤"
                )
                time.sleep(100)
            else:
...
@bot.on(events.NewMessage(incoming=True, from_users=(1718517513)))
async def hehehe(event):
    ...
    if event.is_private:
        if not pm_sql.is_approved(chat.id):
            pm_sql.approve(
                chat.id, f"**My Boss iz here..{jarvis_mention}'s Its Ur Lucky day Nibba😏!!**"
            )
